...
// --- 2. 工具函數 ---
function generateSmartOptions(targetWord, allWords) {
  ...
  interferencePool.sort(() => 0.5 - Math.random());
  const finalOptions = [...interferencePool.slice(0, 3), targetWord].sort(
    () => 0.5 - Math.random()
  );
  return finalOptions;
}
...
function QuizScreen({ mode, wrongWords, setWrongWords, setActiveTab }) {
  ...
  const generateQuestion = () => {
    ...
    const answer = shuffledWords[0];
    const choices = generateSmartOptions(answer, wordDatabase);
...
// --- 4. 我的單字庫畫面元件 ---
function VocabBankScreen({ wrongWords, setActiveTab }) {
